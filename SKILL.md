...
A zero-retention, VLM-powered image composition skill for OpenClaw Agents.

## Features
- **Master Photography VLM**: Leverages `gpt-4o` Vision capabilities to detect subjects and map them to classical composition frameworks (Rule of Thirds, Golden Ratio, etc.).
...
2. Call the `master_crop` tool with the file path (ensure `delete_after` is true).
3. The tool will parse the image, delete the original file, and return the optimal bounding box, the compositional rule applied, and the `image_data_uri`.
